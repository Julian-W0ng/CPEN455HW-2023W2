...
def train_or_test(model, data_loader, optimizer, loss_op, device, args, epoch, mode = 'training'):
    ...
    for batch_idx, item in enumerate(tqdm(data_loader)):
        ...
        if mode == 'training' or mode == 'val':
            ...
            model_out = model(model_input, categories)
            loss = loss_op(model_input, model_out)
...
if __name__ == '__main__':
    ...
    if "mnist" in args.dataset:
        ...
        data_augmentation_transforms = transforms.Compose([transforms.Resize((32, 32)), 
    ...
    for epoch in tqdm(range(args.max_epochs)):
        ...
        if epoch % args.sampling_interval == 0:
            print('......sampling......')
            sample_t = sample(model, args.sample_batch_size, args.obs, sample_op, num_classes=len(my_bidict))
